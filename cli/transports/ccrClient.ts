...
import { secureJitterFraction, secureJitterMs } from 'src/utils/secureJitter.js'
  ...
  private startHeartbeat(): void {
    ...
    const schedule = (): void => {
      const jitter =
        ...
        secureJitterFraction() // SEC-FIX: crypto.randomInt()
  ...
  private async getWithRetry<T>(
    ...
    for (let attempt = 1; attempt <= 10; attempt++) {
      ...
      try {
        ...
        if (attempt < 10) {
          const delay =
            Math.min(500 * 2 ** (attempt - 1), 30_000) + secureJitterMs(500) // SEC-FIX: crypto.randomInt()
          await sleep(delay)
        }
        continue
      }
